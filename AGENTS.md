...
6. **AI lives in `apps/agent`** (Mastra). The product app (`apps/edubridge`) uses the typed client with scope-claimed service tokens; no LLM calls in the Next app. Agents **never write tenant data directly** — they draft, humans approve, writes go through server actions. Memory is resource-scoped per user. Hosted agent is a **second Coolify service**, not Mastra Cloud ([ai-platform.md](docs/architecture/ai-platform.md#where-it-runs)). Details: `docs/architecture/agent-ecosystem.md`.
...
15. **Deployment environments:** Next.js owns `NODE_ENV` (`development` on `pnpm dev`, `production` on Vercel and Coolify). Branch name does not change product gates — use `APP_ENV` only for hostname/routing (`local` / `staging` / `production`). Cursor: `.cursor/rules/55-deployment-environments.mdc`. Architecture: `docs/architecture/deployment-environments.md`.
