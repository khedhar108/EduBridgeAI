...
4. **Drizzle owns table access.** No raw SQL in app code; Supabase client only for auth/storage/realtime. Every action: resolve session → assert role → `withTenant(...)` → query. RLS is the backstop, not the UX.
