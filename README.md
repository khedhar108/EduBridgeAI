...
3. **Non-negotiable conventions** (details in [docs/roadmap/README.md](./docs/roadmap/README.md#cross-phase-standards)):
   - Every tenant table carries `school_id` with Supabase **RLS enabled**; role checks are server-side (eight roles: `platform_owner`, `school_admin`, `coordinator`, `accountant`, `teacher`, `staff`, `student`, `parent`).
